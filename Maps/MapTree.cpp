 ...
 * Copyright (C) 2011 LastDawn <http://www.lastdawn.net/>
...
namespace VMAP
{
    ...
    class AreaInfoCallback
    {
        public:
            ...
            {
#ifdef VMAP_DEBUG
                printf( "AreaInfoCallback: trying to intersect '%s'", prims[entry].name.c_str());
#endif
    ...
    class LocationInfoCallback
    {
        public:
            ...
            {
#ifdef VMAP_DEBUG
                printf("LocationInfoCallback: trying to intersect '%s'", prims[entry].name.c_str());
#endif
    ...
    */
    ...
    {
        printf( "StaticMapTree::InitMap() : initializing StaticMapTree '%s'", fname.c_str());
        bool success = true;
        ...
        else
        {
            ...
            ModelSpawn spawn;
#ifdef VMAP_DEBUG
            printf( "StaticMapTree::InitMap() : map isTiled: %u", static_cast<uint32>(iIsTiled));
#endif
                ...
                if (result)
                {
                    ...
                    if (fread(&referencedVal, sizeof(uint32), 1, tf) == 1)
                    {
                        if (!iLoadedSpawns.count(referencedVal))
                        {
                            ...
                            {
                                printf("StaticMapTree::LoadMapTile() : invalid tree element (%u/%u)", referencedVal, iNTreeValues);
                                continue;
                            }
#endif
                        ...
                        else
                        {
                            ...
                            if (iTreeValues[referencedVal].ID != spawn.ID)
                                ...
                                printf("StaticMapTree::LoadMapTile() : name collision on GUID=%u", spawn.ID);
#endif
                        }
                    }
                    else
